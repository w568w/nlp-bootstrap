...
class ESIM(nn.Module):
    ...
    def forward(self, x: Tensor):
        ...
        a_bar, _ = self.input_encoding[0](x[0])  # (batch_size,word_num,2*lstm_hidden_size)
        b_bar, _ = self.input_encoding[1](x[1])  # (batch_size,word_num,2*lstm_hidden_size)
        ...
        v_a, _ = self.composition[0](m_a)  # (batch_size,word_num,lstm_hidden_size*2)
        v_b, _ = self.composition[1](m_b)  # (batch_size,word_num,lstm_hidden_size*2)
...
def main():
    ...
    x, y = generate_vectors(glove, data[:1000])
    x = torch.from_numpy(x).to(device)
    y = torch.tensor(y).to(device)
    per = 800
    batch_size = 10

    model = ESIM(glove.vectors, x.shape[-1], 600, 3).to(device)
